...
- `PlayZInfected` — CE spawn-dead roll: configurable % of infected spawn already dead. Config: `$profile:PlayZ/Infected.json` (`Enabled`, `SpawnDeadChancePercent`, default 15). Requires `InediaInfectedAI`; load after `@InediaInfectedAI` and `@PlayZInedia`.
...
- `PlayZ129TentFix` — DayZ 1.29 tent open/close/pack fix (`ActionTargets` keeps `TentBase` NOID proxies). Pack into `@PlayZ_Client`; remove when Bohemia restores tent proxy targeting.
...
- `PlayZNoBase` — blocks crafting `FenceKit`, `WatchtowerKit`, and `TerritoryFlagKit` (recipe `CanDo` false). Looted/spawned kits still deploy. Load after Expansion Base Building if that workshop module is on the mod line. Just to disable Vanilla Base Building only.
...
- `PlayZLock` — remembered combo-lock open (owner/guest), dial HUD, simulated dial animation, manage/remove, admin open/remove. Vanilla `CombinationLock` / `CombinationLock4` only (no extra lock items, no boltcutter raid). Config: `$profile:PlayZ/Lock.json` (`ClientSyncConfig`, `ServerConfig`, `AdminIDs[]`) synced to clients on connect. Requires `JM_CF_Scripts` (CF ModStorage for owner/guest persistence). Based on ZenComboLocks by Zenarchist. Load after `CF`.
- `PlayZLocktRPatch` — PlayZLock ↔ Tierro tR gates/doors (`tR_Stockade`, `tR_StockadeCorner`, `tR_NarrowDoor`, `tR_MediumDoor`, `tR_Shed`): InteractAnywhere actions, open/close helper, `IsLockedOnGate` / `IsLockAttached` for tR parents. Requires `PlayZLock` + `tR_Custom` / `tR_Stockade_Custom` / `tR_Modules_Custom`. Load after `@PlayZLock` and upstream `@tR_*` + `tR_Custom.pbo`.
- `PlayZtRPatch` — restores vanilla hologram collision for all deploy/place (barrels, tents, `tR_*`, …). Softens tR↔tR contact so stockade snap stays usable; hides deploy when colliding. Requires `tR_*` / `tR_Custom`, Expansion BaseBuilding scripts, `PlayZNotes`. Load after those. Pack via **PlayZ: Build Client PBO** → `PlayZtRPatch`.
