...
- `PlayZExpansion` — Sakhal Expansion compatibility patches (not a fork of Expansion). Requires `DayZ-Expansion-Core` + Market + TerjeStartScreen in `-mod=` **after** Expansion, **with** `@PlayZExpansion` loaded after `@PlayZUI` on the mod line. `NailBox` quantity shim; dead-player **Statistics** on PlayZ ingame menu; corpse cursor uses Terje identity (suppresses vanilla `Survivor` when `DisplayPlayerNamesMode` blocks name, e.g. masked face).
