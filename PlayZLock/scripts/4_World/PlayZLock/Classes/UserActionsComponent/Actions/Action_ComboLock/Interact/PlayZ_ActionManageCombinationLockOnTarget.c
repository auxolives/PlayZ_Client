class PlayZ_ActionManageCombinationLockOnTarget : ActionInteractBase
{
	...
	override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		if (!target.GetObject() || !PlayZLockConfig.Get().ClientSyncConfig || (player.IsPlayZLockAdmin() && !PlayZLockConfig.Get().ClientSyncConfig.AllowAdminOpen))
			return false;
		...
		return !PlayZLockHelper.IsOpen(lock.GetHierarchyParent()) && lock.IsPermittedToOpen(player) && !lock.IsTakeable();
	}
